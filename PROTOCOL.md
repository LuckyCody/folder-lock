...
3. Every turn ends by RUNNING the signoff (`scripts/signoff.py`; `--turn` for a mid-task stop on a question to the owner; `--held none` when nothing is held). It writes the session's **signoff record** (`<sessions>/<sid>.signoff.json`) — the terminal block as data, which the Stop hook checks against the locks on disk — and prints the **closing message**, the only thing the chat shows (v5.3, owner ruling 2026-09-21): one plain line per item that waits on the owner and per direct answer to the owner's questions, no commands, paths or ids; when nothing waits on the owner exactly `Nothing needed from you.`. The block itself, verbatim structure, lives in the record and the pointer (a fired agent's final reply still ends with its `DONE | WAITING_CODY | FAILED: …` line):
   ```
...
| `require_signoff.py` (Stop) | end of every turn | a turn without a signoff record from THIS turn (`run the signoff — …`), a record whose `held` disagrees with the locks on disk, a `closing` lock with a stale pointer or an unreleased `LOCK.yaml`. The chat text is not inspected (v5.3). |
