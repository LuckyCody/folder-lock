...
**v5.2.0** — the board becomes a **materialized view** (lib/boardmat.py): every state writer (claim, handoff, answer, signoff, release, new) ends with `materialize`, which derives the view once and writes `board/board.json` + `board/board.md` into the state store; `board.py glance` then reads that ONE object — no lock, no kick, no entry guards, one touch (the digest window) — and re-renders live only when the object's `source_stamps` no longer match the `items` etag + the lock-identity fingerprint. Answering a `waiting_owner` item is now LOCKLESS (`board.py answer <handle> "<text>"` files a type `answer` handoff and re-readies the item; `answer --batch` parses the anchors), and the live-store path sits behind an env fuse — a patched `statestore.BACKEND` attribute can never reach a blob store, `items._live_store()` is the one predicate a destructive path asks. Board tests 13/13. See [What's new in v5.2](#whats-new-in-v52).
...
The board used to be derived live on every look — a full tree walk, `items.sync`, a state-store read, the digest: seconds in a fresh window. The board is a pure view over the state documents and the lock tree, so it is now materialized by every writer of that state and a look is a single read.

- **`lib/boardmat.py`** — `materialize(trigger)` derives the view (`board.build_view` + `items.sync` — nothing re-implemented), writes `board/board.json` + `board/board.md` through `statestore.raw_put` (no merge, no outbox — a derived view, last writer wins) and caches locally. `fetch()`/`is_fresh()` compare the object's `source_stamps` — the `items` etag and a **lock identity fingerprint** (folder, kind, window, status) — never a lock file's mtime or etag: a firing lock is heart-beaten every minute, so a time stamp would leave the board stale for as long as any agent runs. A materialize failure never blocks the caller.
- **The three verbs** (`scripts/board.py` + `scripts/lock.py`):
  - `glance` — ONE raw GET of the materialized board: no lock, no kick, no entry guards, one state write (the touch). The cheap path a session starts on; stale or missing → a live render + `materialize` (`render_fallback`).
  - `claim` — the entry guards live HERE (the §15 claim gate: sync-conflict folding, rules-hash publish). A glance never runs them.
  - `answer` — LOCKLESS: files a type `answer` handoff into the item's folder inbox, re-readies the asking item (`items.answer` + the `answered_at` hold in `sync`), then materializes. `answer --batch` parses `<!-- answer: <handle> -->` anchors. Exit 0 landed · 2 no text · 3 not waiting_owner · 4 unknown · 5 the note could not be staged.
