...
4. `python <skill>/scripts/signoff.py --decisions "<defaults you decided, or none>" [--answer "<plain answer to a question the owner asked>"]...` (v5.3) — from-pointer, `status:`/`next:` from the board, the archive decision (`.goal/goal.md` `complete: true` + 0 open items → `_archive/<slug>/`, registry `archived: true`; never a delete), `lock.py release` (a refusal is printed verbatim and NO record is written: the turn may not end), the autorun-log line, `board.py --signpost`. It ends by writing the **signoff record** (the terminal block as data — what the Stop hook reads) and printing TWO outputs. The block, for the pointer and the loop, never for the chat:
   ```
   status: done | blocked | handed-off
   held:   <folder> | none
   next:   <exact command> | none — waiting on the owner
   ```
   …and, LAST, the **closing message** — the ONLY thing you paste into the chat: one plain-English line per item that waits on the owner and per direct answer to a question the owner asked (`--answer`); no commands, no paths, no ids, no counts. When nothing waits on the owner it is exactly `Nothing needed from you.` Anything the agent or the loop can do itself (deploys, verification clicks, mails to third parties) never appears in it — that goes to the pointer. A session that claimed nothing runs `signoff.py --held none` — "nothing is held" is a lock state, not a session end. A MID-TASK turn that ends on a question to the owner runs `python <skill>/scripts/signoff.py --turn --ask "<the question>"` — record + closing message, nothing released.
...
- Ending a turn without running the signoff (full, `--turn`, or `--held none`), or pasting the terminal block, commands, paths or ids into the chat — the chat shows only the closing message (v5.3).
...
hooks/require_signoff.py     Stop hook: every turn must carry a signoff record from THIS turn (`held` checked against the locks); no ending a turn with a closing lock; the chat is never inspected
...
scripts/signoff.py           the signoff tail: status/next from the board, archive decision, release, autorun-log line, the signoff record + the closing message; --held none; --turn for a mid-task stop (v5.3)
